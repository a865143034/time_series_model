...
class LSTM_Regression(nn.Module):
    ...
    def attention_net(self, lstm_out, final_state,hidden_size,lstm_output):
        hidden = final_state.view(-1, hidden_size,
...
if __name__ == '__main__':
    ...
    plt.plot([i for i in range(len(dataset_x))],dataset_y,'b',label='real')
    plt.plot([i+train_size for i in range(len(test_x))],actual_pred_y[-len(test_y):], 'r', label='prediction')
    #plt.plot(test_y, 'b', label='real')
    #plt.plot((train_size, len(test_y)), (0, 1), 'g--')  # 分割线 左边是训练数据 右边是测试数据的输出
    plt.vlines(train_size, 0, 60000, colors = "g", linestyles = "dashed")
    plt.legend(loc='best')
